...
display(f::jutulForce{D, T}) where {D, T} =
    println("$(D)D jutulForce structure with $(length(f.loc)) injection/production wells and rate $(f.irate)")

function force(M::jutulModel{D, T}, f::jutulForce{D, T}; ρCO2::Number=T(501.9)) where {D, T}
    model = model_(M)
