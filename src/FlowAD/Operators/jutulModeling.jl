...
function (S::jutulModeling{D, T})(Transmissibilities::AbstractVector{T}, f::jutulForce{D, N};
    state0::jutulInitState{T}=jutulInitState(S.model), visCO2::T=T(1e-4), visH2O::T=T(1e-3), ρCO2::T=T(501.9), ρH2O::T=T(1053.0), info_level::Int64=-1) where {D, T, N}
    ...
    states, rep = simulate(dict(state0), model, day * S.tstep, parameters = parameters, forces = forces, info_level = info_level, max_timestep_cuts = 1000)
